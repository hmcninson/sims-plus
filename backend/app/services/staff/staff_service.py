...
Staff CRUD, ID generation, stats, CSV import, class assignments,
and teaching staff methods.
"""
...
class StaffService:
    ...
    async def create_staff(
        ...
        # Reload with school relationship for the endpoint response
    ...
    async def get_staff(
        self, tenant_id: UUID, staff_id: UUID, include_assignments: bool = False
    ) -> Optional[Staff]:
        """Get staff by ID with school and optional assignments."""
        ...
        # Always load school for the detail view (endpoint accesses staff.school.name)
        query = query.options(selectinload(Staff.school))
        if include_assignments:
            # Chain class_ loading so endpoint can access assignment.section.class_.name
            query = query.options(
    ...
    async def list_staff(
        ...
        if department:
            conditions.append(Staff.department.ilike(f"%{escape_ilike(department)}%"))
        if school_id:
    ...
    async def delete_staff(self, tenant_id: UUID, staff_id: UUID) -> bool:
    ...
    async def assign_staff_to_section(
        ...
        # Reload with section and class relationships for the endpoint response
        result = await self.db.execute(
            select(StaffClassAssignment)
            .where(StaffClassAssignment.id == assignment.id)
            .options(
    ...
    async def update_staff_assignment(
        ...
        **kwargs,
        ...
        reload_result = await self.db.execute(
            select(StaffClassAssignment)
            .where(StaffClassAssignment.id == assignment_id)
            .options(
                selectinload(StaffClassAssignment.section).selectinload(ClassSection.class_),
            )
        )
    ...
    async def get_staff_assignments(
        ...
        result = await self.db.execute(
            ...
            .options(
                # Chain class_ loading so endpoint can access a.section.class_.name
                selectinload(StaffClassAssignment.section).selectinload(ClassSection.class_),
            )
        )
