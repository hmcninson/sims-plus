...
async def get_dashboard_stats(
    ...
    """Get overview statistics (cached 60s)."""
    redis = getattr(request.app.state, "redis", None)
    cache = CacheService(redis)

    async def fetch_stats():
        service = DashboardService(db)
        return await service.get_overview_stats(tenant.tenant_id)

    data = await cache.get_or_set(
        CacheKeys.dashboard_stats(str(tenant.tenant_id)),
...
async def get_attendance_trend(
    ...
    """Get daily attendance trend for last N days."""
    service = DashboardService(db)
    data = await service.get_attendance_trend(tenant.tenant_id, days=days)
...
async def get_fee_collection_trend(
    ...
    service = DashboardService(db)
    data = await service.get_fee_collection_trend(tenant.tenant_id, months=months)
...
async def get_class_performance(
    ...
    """Get average exam scores per class for a term."""
    service = DashboardService(db)
    data = await service.get_class_performance(tenant.tenant_id, term_id)
...
async def get_gender_distribution(
    ...
    service = DashboardService(db)
    data = await service.get_gender_distribution(tenant.tenant_id)
