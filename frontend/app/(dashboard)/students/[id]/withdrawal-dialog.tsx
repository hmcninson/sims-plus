...
export function WithdrawalDialog({
  ...
  // Check outstanding fees when dialog opens
  useEffect(() => {
    if (open) {
      setStep(1);
      setFeeCheck(null);
  ...
  // Step 1: Submit withdrawal form
  async function onSubmitForm(values: WithdrawalFormData) {
    setIsLoading(true);
    const result = await initiateWithdrawal(studentId, {
  ...
  async function handleClearanceToggle(
    field: "library_cleared" | "finance_cleared" | "property_cleared" | "boarding_cleared",
    value: boolean
  ) {
    if (!clearance) return;
    setIsLoading(true);
  ...
  // Step 2 -> 3: Complete withdrawal
  async function handleComplete() {
    setIsLoading(true);
    const result = await completeWithdrawal(studentId);
    ...
    if (result.success) {
      ...
      toast.error(result.error || "Failed to complete withdrawal");
    }
    setIsLoading(false);
  }

  // Download withdrawal letter (same as transfer certificate endpoint)
  async function handleDownloadLetter() {
    ...
    if (result.success && result.data) {
      ...
      a.href = url;
      a.download = `withdrawal-letter-${studentId}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast.success("Withdrawal letter downloaded");
    } else {
      toast.error(result.error || "Failed to download letter");
    }
    setIsLoading(false);
  }
  ...
  // Format today for min date attribute
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          <DialogTitle>
            {step === 3 ? "Withdrawal Complete" : "Withdraw Student"}
          </DialogTitle>
          <DialogDescription>
            {step === 1 && `Initiate withdrawal process for ${studentName}.`}
            {step === 2 && "Complete the clearance checklist before finalizing."}
            {step === 3 && `${studentName} has been withdrawn successfully.`}
          </DialogDescription>
        </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Date when the withdrawal takes effect (DD/MM/YYYY)
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-3">
                      ...
                      <div className="space-y-1 leading-none">
                        ...
                        <FormDescription>
                          Allow withdrawal despite outstanding fees
                        </FormDescription>
                      </div>
                    </FormItem>
                  )}
          ...
          <div className="space-y-4">
            <Card className="border-green-200 dark:border-green-800">
              <CardContent className="pt-6 text-center">
                ...
                <p className="font-medium text-lg">Withdrawal Complete</p>
                <p className="text-sm text-muted-foreground mt-1">
                  {studentName} has been successfully withdrawn.
                </p>
              </CardContent>
            </Card>
